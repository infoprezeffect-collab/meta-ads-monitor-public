...
import io
import json
import smtplib  # kept for compatibility
import logging
import requests
...
def build_email_html(ai_report: str, data: dict, date_str: str) -> str:
    ...
    <tr style="background:#1a1a2e;">
        ...
        <th style="padding:10px 14px; text-align:right; color:white; font-size:12px;">HIER (J-1)</th>
        <th style="padding:10px 14px; text-align:center; color:#a8b2d8; font-size:11px;">vs Avant-hier</th>
        <th style="padding:10px 14px; text-align:right; color:#a8b2d8; font-size:12px;">AVANT-HIER (J-2)</th>
  ...
  <tr>
    <td style="background:#ffffff; padding:28px 40px; border-left:1px solid #e2e8f0; border-right:1px solid #e2e8f0;">
      <h2 style="margin:0 0 16px; color:#1a1a2e; font-size:15px; font-weight:700; text-transform:uppercase; letter-spacing:1px;">
        📈 KPI Comparatifs — Hier (J-1) vs Avant-hier (J-2) vs Moy. 7 jours
      </h2>
...
def send_email(html_content: str, date_str: str, subject_prefix: str = "📊 Rapport Meta Ads", filename_prefix: str = "rapport_meta_ads"):
    ...
    payload = {
        "from": "Meta Ads Monitor <onboarding@resend.dev>",
        "to": [EMAIL_SENDER],
    ...
    if not resp.ok:
        log.error(f"Erreur Resend : {resp.status_code} {resp.text}")
        resp.raise_for_status()
